...
import { CheckCircle, ArrowRight, Bot, ShieldAlert } from 'lucide-react';
...
export default function CalibratePage() {
    ...
    try {
      ...
      getBestMove(chess.fen()).then((best) => {
        if (best && best !== '(none)') {
          chess.move(best);
          setFen(chess.fen());
          if (chess.isGameOver()) endCalibrationGame(chess.isCheckmate() ? 'loss' : 'draw');
        }
        setThinking(false);
      });
      return true;
  ...
  function onSquareClick(square: any) {
    ...
    const moveSucceeded = handleMove({ from: moveFrom, to: square, promotion: 'q' });
    setMoveFrom(null);
    setOptionSquares({});
  }
  ...
  if (!isVerified) {
    return (
      <div className="max-w-md mx-auto px-4 py-20 text-center space-y-6">
        ...
        <p className="text-ink-400">You must click the link in your email and sign in before starting calibration.</p>
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 page-enter">
      <div className="text-center mb-8">
        ...
        <p className="text-ink-400">Play {gamesRemaining} more game{gamesRemaining !== 1 ? 's' : ''} to determine your rating.</p>
      </div>

      <div className="max-w-lg mx-auto mb-8">
        <div className="flex gap-2 justify-center mb-3">
          {Array.from({ length: 5 }).map((_, i) => (
            <div key={i} className={`h-2 flex-1 rounded-full ${i < gamesPlayed ? 'bg-gold' : i === gamesPlayed ? 'bg-gold/40 animate-pulse' : 'bg-ink-700'}`} />
          ))}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-6">
        <div className="space-y-3">
          ...
          {gameResult && !submitting && (
            <div className={`card p-4 text-center border ${gameResult === 'win' ? 'border-green-700 bg-green-900/20' : gameResult === 'loss' ? 'border-red-800 bg-red-900/20' : 'border-ink-600'}`}>
              <div className="font-bold text-sm mb-3 capitalize">{gameResult === 'win' ? 'You Won!' : gameResult === 'loss' ? 'Bot Won' : 'Draw!'}</div>
              {gamesRemaining > 0 && <button onClick={startGame} className="btn-gold btn-sm">Next Game <ArrowRight size={12} /></button>}
            </div>
          )}
          {submitting && <div className="text-center text-ink-400 text-sm animate-pulse">Saving results...</div>}
        </div>
          ...
          <div className="card p-4">
            ...
            <div className="space-y-3 text-xs text-ink-400 leading-relaxed">
              ...
              <div className="space-y-1.5">
                <div className="flex items-center gap-2"><span className="text-green-400">Win &rarr;</span> Next bot is harder</div>
                <div className="flex items-center gap-2"><span className="text-red-400">Loss &rarr;</span> Next bot is easier</div>
              </div>
            </div>
          </div>
        </div>
