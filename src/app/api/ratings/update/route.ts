...
export async function POST(req: NextRequest) {
  if (req.headers.get('x-internal-secret') !== process.env.INTERNAL_API_SECRET) {
    console.error('[ratings/update] Unauthorized — check INTERNAL_API_SECRET matches on Render and Vercel');
  ...
  // ── BUG 2 FIX: games_played could be null for new players ────────────────────
  // null + 1 = NaN in JS, which JSON.stringify turns into null.
    ...
    await supabase.from('rating_history').insert([
      {
        ...
        rating:           updates.white.newRating,
        rating_deviation: updates.white.newRD,
        volatility:       updates.white.newVolatility,
        change:           updates.white.ratingChange,
        recorded_at:      now,
      },
      {
        player_id:        black.id,
        game_id,
        season:           game.season ?? null,
        rating:           updates.black.newRating,
        rating_deviation: updates.black.newRD,
        volatility:       updates.black.newVolatility,
        change:           updates.black.ratingChange,
        recorded_at:      now,
      },
    ]);
  } catch (rhErr: any) {
