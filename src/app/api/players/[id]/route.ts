...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: player }, { data: standings }, { data: recentGames }, { data: ratingHistory }] = await Promise.all([
    ...
    supabase.from('games').select(`
      id, result, league, tier, played_at, white_player_id, time_control,
    ...
    `).or(`white_player_id.eq.${id},black_player_id.eq.${id}`).neq('result', '*').neq('league', 'calibration').order('played_at', { ascending: false }).limit(30),
