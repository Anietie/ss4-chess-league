...
const PIECE_SYMBOLS: Record<string, string> = { p: '♙', n: '♘', b: '♗', r: '♖', q: '♕', P: '♟', N: '♞', B: '♝', R: '♜', Q: '♛' };
  ...
  const captured: string[] = useMemo(() => {
    ...
    for (const move of history) {
      if (move.captured) {
        ...
        if (capturedColor === (color === 'white' ? 'b' : 'w')) pieces.push(move.captured);
      }
    }
...
function Clock({ name, rating, timeMs, isActive, color, chess, playerColor }: ClockProps): React.ReactElement {
  ...
  useEffect(() => {
    setDisplay(timeMs);
    if (isActive) { lastRef.current = Date.now(); ref.current = setInterval(() => { const now = Date.now(); const el = now - lastRef.current; lastRef.current = now; setDisplay((p: number) => Math.max(0, p - el)); }, 100); }
...
export default function GameRoomPage(): React.ReactElement {
  ...
  // ── REVIEW-STYLE NAVIGATION STATE ────────────────────────────────────────
  const [reviewFens, setReviewFens] = useState<string[]>([]);
  const [reviewIdx, setReviewIdx] = useState<number>(0);
  const [isReviewing, setIsReviewing] = useState<boolean>(false);
  const [reviewLastMove, setReviewLastMove] = useState<Record<string, React.CSSProperties>>({});
  ...
  useEffect(() => { if (!claimGraceEndsAt) { setClaimCountdown(0); return; } const update = () => { setClaimCountdown(Math.max(0, Math.floor((new Date(claimGraceEndsAt).getTime() - Date.now()) / 1000))); }; update(); const t = setInterval(update, 1000); return () => clearInterval(t); }, [claimGraceEndsAt]);
  ...
  useEffect(() => {
    ...
    const fens = [temp.fen()];
    for (const m of moves) { try { temp.move(m); fens.push(temp.fen()); } catch { break; } }
    setReviewFens(fens);
    if (!isReviewing) setReviewIdx(fens.length - 1);
  }, [moves, isReviewing]);

  // Highlight last move square when at latest position
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    async function init(): Promise<void> {
      try {
        ...
        if (typeof window !== "undefined") resolvedPlayerId = localStorage.getItem("player_id") ?? undefined;
        if (!resolvedPlayerId) { const { data: byAuthId } = await supabase.from("players").select("id").eq("auth_user_id", user.id).maybeSingle(); if (byAuthId?.id) { resolvedPlayerId = byAuthId.id; localStorage.setItem("player_id", resolvedPlayerId ?? ""); } }
        ...
        sock.on("game_state", (d: any) => {
          if (d.pgn) chess.loadPgn(d.pgn); setFen(d.fen || chess.fen()); setMoves(chess.history());
        ...
        sock.on("game_started", (d: any) => { setStatus("active"); if (myPlayerIdRef.current === d.white_player_id) myColorRef.current = "white"; if (myPlayerIdRef.current === d.black_player_id) myColorRef.current = "black"; setWhite(p => ({ ...p, name: d.white_name || p.name, rating: d.white_rating || p.rating, timeMs: d.white_time, isActive: true })); setBlack(p => ({ ...p, name: d.black_name || p.name, rating: d.black_rating || p.rating, timeMs: d.black_time, isActive: false })); });
        sock.on("move_made", (d: any) => {
          if (d.pgn) chess.loadPgn(d.pgn); else chess.load(d.fen); const history = chess.history({ verbose: true }); const lm = history[history.length - 1];
          ...
          setWhite(p => ({ ...p, timeMs: d.white_time, isActive: d.current_turn === "w" })); setBlack(p => ({ ...p, timeMs: d.black_time, isActive: d.current_turn === "b" }));
        ...
        sock.on("game_ended", (d: any) => { setStatus("ended"); setResult(d.result); setConnectionStatus("connected"); setOpponentStatus("connected"); if (d.pgn) { setFinalPgn(d.pgn); try { chess.loadPgn(d.pgn); setFen(chess.fen()); setMoves(chess.history()); } catch {} } setWhite(p => ({ ...p, isActive: false })); setBlack(p => ({ ...p, isActive: false })); if (myColorRef.current) { const iWon = (d.result === "1-0" && myColorRef.current === "white") || (d.result === "0-1" && myColorRef.current === "black"); if (iWon) playRef.current("win"); else if (d.result === "0.5-0.5") playRef.current("draw"); else playRef.current("loss"); } });
  ...
  function getMoveOptions(square: Square): boolean {
    ...
    moves.forEach((m) => { newSquares[m.to] = { background: chess.get(m.to) ? "radial-gradient(circle, rgba(0,0,0,.1) 85%, transparent 85%)" : "radial-gradient(circle, rgba(0,0,0,.1) 25%, transparent 25%)", borderRadius: "50%" }; });
    newSquares[square] = { background: "rgba(255, 255, 0, 0.4)" }; setOptionSquares(newSquares); return true;
  }
  function tryMoveLocally(from: Square, to: Square, promotion: string = "q"): string | null { const t = new Chess(chess.fen()); try { const r = t.move({ from, to, promotion }); return r ? t.fen() : null; } catch { return null; } }
  function emitMove(from: Square, to: Square, promotion: string = "q"): void { socketRef.current?.emit("make_move", { game_id: gameId, player_id: myPlayerIdRef.current, move: { from, to, promotion } }); }
  ...
  const onDrop = useCallback((src: string, tgt: string): boolean => {
    ...
    const from: Square = src as Square; const to: Square = tgt as Square; const newFen = tryMoveLocally(from, to);
  ...
  if (status === "unauthorized") return (<div className="min-h-[60vh] flex items-center justify-center px-4"><div className="card p-8 w-full max-w-sm text-center space-y-4"><ShieldAlert className="mx-auto text-red-400" size={48} /><h2 className="text-xl font-bold text-chalk">Authentication Required</h2><p className="text-ink-400 text-sm">You must verify your email and sign in before you can participate in a live match.</p><button onClick={() => router.push("/auth/login")} className="btn-gold w-full text-sm">Sign In</button></div></div>);
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6">
      {connectionStatus === "reconnecting" && (<div className="fixed top-20 left-1/2 -translate-x-1/2 z-50 bg-amber-900/95 border border-amber-500/60 px-6 py-3 rounded-xl shadow-2xl backdrop-blur-sm"><div className="flex items-center gap-3"><Loader2 className="animate-spin text-amber-400 flex-shrink-0" size={18} /><div><div className="font-semibold text-amber-300 text-sm">Reconnecting...</div><div className="text-xs text-amber-400/80">Attempting to restore connection</div></div></div></div>)}
      {connectionStatus === "disconnected" && status !== "ended" && (<div className="fixed top-20 left-1/2 -translate-x-1/2 z-50 bg-red-900/95 border border-red-500/60 px-6 py-3 rounded-xl shadow-2xl backdrop-blur-sm"><div className="flex items-center gap-3"><WifiOff size={18} className="text-red-400" /><div><div className="font-semibold text-red-300 text-sm">Connection Lost</div><div className="text-xs text-red-400/80">You have 3 minutes to reconnect.</div></div></div></div>)}
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          <span className={`flex items-center gap-1.5 text-xs px-2 py-0.5 rounded-full border ${connectionStatus === "connected" ? "border-green-700/50 text-green-400 bg-green-400/10" : "border-red-700/50 text-red-400 bg-red-400/10"}`}>{connectionStatus === "connected" ? <Wifi size={11} /> : <WifiOff size={11} />}{connectionStatus}</span>
          {!isSpectator && (<button onClick={() => { const n = !confirmMove; setConfirmMove(n); localStorage.setItem("confirmMove", String(n)); if (!n) { setPendingMove(null); setPendingFen(null); } }} className={`text-xs px-2 py-1 rounded border ${confirmMove ? "border-orange-500 text-orange-400 bg-orange-500/10" : "border-ink-600 text-ink-400 hover:text-chalk"}`}>✓ Confirm</button>)}
      ...
      {opponentStatus === "disconnected" && status === "active" && (<div className="mb-4 p-4 rounded-xl bg-red-900/20 border border-red-700/50"><div className="flex items-center gap-3"><WifiOff size={18} className="text-red-400 flex-shrink-0" /><div className="flex-1"><div className="text-sm font-medium text-red-300">{disconnectedPlayerName || "Opponent"} Disconnected</div>{reconnectCountdown !== null && reconnectCountdown > 0 && <div className="text-xs text-red-400/80 mt-1">Automatic forfeit in {Math.floor(reconnectCountdown / 60)}:{(reconnectCountdown % 60).toString().padStart(2, "0")}</div>}</div><NudgeButton onClick={nudgeOpponent} disabled={nudgeCooldown > 0} label={nudgeCooldown > 0 ? `${nudgeCooldown}s` : "Nudge"} /><button onClick={claimNoShow} className="btn-danger btn-sm gap-1.5"><AlertTriangle size={13} />Claim Forfeit</button></div></div>)}

      {drawOffered && (<div className="mb-4 p-4 rounded-xl bg-orange-500/10 border border-orange-500/30 flex items-center justify-between"><span className="text-sm text-chalk">Draw offered - accept?</span><div className="flex gap-2"><button onClick={() => { socketRef.current?.emit("accept_draw", { game_id: gameId }); setDrawOffered(false); }} className="btn-gold btn-sm">Accept</button><button onClick={() => setDrawOffered(false)} className="btn-ghost btn-sm">Decline</button></div></div>)}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-6">
        <div className="space-y-3">
          ...
          <div className="board-wrapper touch-none select-none relative">
            {typeof window !== "undefined" && (<Chessboard position={boardFen} onPieceDrop={onDrop} onSquareClick={onSquareClick} customSquareStyles={boardSquares} boardOrientation={orientation} customDarkSquareStyle={{ backgroundColor: "#4a6080" }} customLightSquareStyle={{ backgroundColor: "#b0bcce" }} arePiecesDraggable={status === "active" && !isSpectator && !touchMode && !pendingMove && connectionStatus === "connected" && !isReviewing} animationDuration={150} />)}
            {isReviewing && (<div className="absolute top-2 left-2 bg-ink-900/90 text-orange-400 text-xs px-2 py-1 rounded border border-orange-500/30">Reviewing move {currentMoveIdx}/{reviewFens.length - 1}</div>)}
          </div>

          {/* Review navigation controls */}
          <div className="flex items-center justify-center gap-1.5 touch-none select-none">
            <button onPointerDown={e => e.preventDefault()} onClick={() => goToReviewPos(0)} disabled={currentMoveIdx === 0} className="btn-ghost p-2 disabled:opacity-30"><ChevronsLeft size={15} /></button>
            <button onPointerDown={e => e.preventDefault()} onClick={() => goToReviewPos(Math.max(0, currentMoveIdx - 1))} disabled={currentMoveIdx === 0} className="btn-ghost p-2 disabled:opacity-30"><ChevronLeft size={15} /></button>
          ...
          {pendingMove && (<div className="flex gap-2 justify-center"><button onClick={() => { emitMove(pendingMove.from, pendingMove.to, pendingMove.promotion); setPendingMove(null); setPendingFen(null); }} className="btn-gold flex-1 text-sm font-semibold gap-1.5">✓ Confirm Move</button><button onClick={() => { setPendingMove(null); setPendingFen(null); }} className="btn-ghost flex-1 text-sm gap-1.5">✕ Cancel</button></div>)}
          ...
          {!isSpectator && status === "active" && !isReviewing && (<div className="flex gap-2"><button onClick={() => socketRef.current?.emit("offer_draw", { game_id: gameId, player_id: myPlayerIdRef.current })} className="btn-ghost flex-1 gap-2 text-sm"><Handshake size={14} /> Offer Draw</button><button onClick={() => confirm("Resign?") && socketRef.current?.emit("resign", { game_id: gameId, player_id: myPlayerIdRef.current })} className="btn-danger flex-1 gap-2 text-sm"><Flag size={14} /> Resign</button></div>)}
          {status === "ended" && (<div className="flex gap-2"><Link href={`/game/${gameId}/review`} className="btn-ghost flex-1 justify-center gap-2 text-sm"><RotateCcw size={14} /> Analysis</Link><button onClick={() => { const pgn = finalPgn ?? chess.pgn(); const a = document.createElement("a"); a.href = URL.createObjectURL(new Blob([pgn], { type: "text/plain" })); a.download = `game-${gameId}.pgn`; a.click(); }} className="btn-ghost flex-1 justify-center gap-2 text-sm"><Download size={14} /> PGN</button></div>)}
        </div>

        <div className="space-y-4">
          <div className="card p-4 space-y-3">
            <div className="section-label mb-1">Opponent Contact</div>
            {(myColorRef.current === "white" ? blackWhatsapp : whiteWhatsapp) && (<a href={`https://wa.me/${formatWhatsAppNumber(myColorRef.current === "white" ? blackWhatsapp : whiteWhatsapp)}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sm text-green-400 hover:text-green-300 py-1.5 px-3 rounded-lg bg-green-400/5 border border-green-400/10 w-full"><MessageCircle size={14} />Message on WhatsApp</a>)}
            {!isSpectator && status === "active" && !isReviewing && (<NudgeButton onClick={nudgeOpponent} disabled={nudgeCooldown > 0 || opponentOnline} label={opponentOnline ? "Opponent is Online" : nudgeCooldown > 0 ? `Nudge again in ${nudgeCooldown}s` : "Nudge Opponent"} />)}
            {!isSpectator && status === "active" && !claimPending && !isReviewing && (<button onClick={claimNoShow} disabled={opponentOnline} className="btn-danger w-full text-sm gap-2 disabled:opacity-40"><AlertTriangle size={14} />{opponentOnline ? "Opponent is Online" : "Opponent No-Show"}</button>)}
            {claimPending && claimCountdown > 0 && (<div className="p-4 rounded-xl bg-amber-900/20 border border-amber-700/50"><div className="flex items-center gap-3"><ClockIcon size={18} className="text-amber-400 flex-shrink-0" /><div className="flex-1"><div className="text-sm font-medium text-amber-300">No-Show Claimed</div><div className="text-xs text-amber-400/80 mt-1">Opponent has {Math.floor(claimCountdown / 60)}:{(claimCountdown % 60).toString().padStart(2, "0")} to join before automatic forfeit</div></div></div></div>)}
          </div>
          ...
          <div className="card p-4"><div className="section-label mb-3">Game Info</div><div className="space-y-2 text-sm"><div className="flex justify-between"><span className="text-ink-400">Status</span><span className="text-chalk capitalize">{status}</span></div><div className="flex justify-between"><span className="text-ink-400">Connection</span><span className={connectionStatus === "connected" ? "text-green-400" : "text-red-400"}>{connectionStatus}</span></div><div className="flex justify-between"><span className="text-ink-400">Opponent</span><span className={opponentStatus === "connected" ? "text-green-400" : "text-red-400"}>{opponentStatus}</span></div><div className="flex justify-between"><span className="text-ink-400">Spectators</span><span className="text-chalk">{spectators}</span></div></div></div>
