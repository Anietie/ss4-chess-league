          ...
          <div className="board-wrapper touch-none select-none relative">
            {typeof window !== "undefined" && (
              <Chessboard
                ...
                arePiecesDraggable={status === "active" && !isSpectator && !touchMode && !pendingMove && connectionStatus === "connected" && !isReviewing}
                animationDuration={150}
              />
            )}
            {isReviewing && (
              <div className="absolute top-2 left-2 bg-ink-900/90 text-orange-400 text-xs px-2 py-1 rounded border border-orange-500/30">
                Reviewing move {currentMoveIdx}/{reviewFens.length - 1}
              </div>
            )}
          </div>
