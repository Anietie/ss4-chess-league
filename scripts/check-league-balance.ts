...
async function checkLeagueBalance() {
  ...
  if (activeLeagues.length < 2) {
    console.log("Need at least 2 active leagues to compare.");
    return;
  }
  ...
  activeLeagues.forEach((l) => {
    const s = stats[l];
    console.log(
      `  ${l.padEnd(12)} avg: ${s.avg}  range: ${s.min}–${s.max}  (${s.count} players)`,
    );
  });
  ...
  if (maxGap > THRESHOLD) {
    ...
    console.log(
      "   Recommended action: Manual inter-league player transfer or adjustment at next draft.\n",
    );
