...
async function finalizeSeason() {
  ...
  if (!winner) {
    console.error("--winner=<player_id> required");
    return;
  }
    ...
    .upsert(
      {
        ...
        season,
        description: `SS4 Champions League Winner Season ${season}`,
      },
      ...
      .upsert(
        {
          ...
          season,
          description: `SS4 CL Runner-Up Season ${season}`,
        },
        { onConflict: "player_id,badge_type,season", ignoreDuplicates: true },
      );
