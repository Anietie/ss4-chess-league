...
async function main() {
  ...
  if (!players?.length) {
    console.log("No players with Chess.com usernames found.");
    return;
  }
  ...
  for (const player of players) {
    ...
    if (player.games_played >= 10) {
      console.log(
        `⏭  ${player.full_name} — skipped (${player.games_played} league games played)`,
      );
      continue;
    }
    ...
    if (!rating) {
      console.log(
        `✗  ${player.full_name} (@${player.chess_com_username}) — API returned nothing`,
      );
      continue;
    }
