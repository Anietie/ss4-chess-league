...
async function rdDecayCron() {
  ...
  const { data: players } = await supabase
    .from("players")
    .select("id, full_name, ss4_rating, rating_deviation, games_played");
  const inactive = (players ?? []).filter(
    (p) => p.games_played < INACTIVE_GAME_THRESHOLD,
  );
  ...
  inactive.forEach((p) => {
    ...
    console.log(
      `  ${p.full_name.padEnd(30)} RD: ${Math.round(p.rating_deviation)} → ${Math.round(newRD)}`,
    );
  });
